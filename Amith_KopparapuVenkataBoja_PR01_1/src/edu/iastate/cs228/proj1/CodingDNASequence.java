...
public class CodingDNASequence extends DNASequence
{
  ...
  */
  public boolean checkStartCodon()
  {
    ...
    for(int i=0;i<3;i++) {
    		if(!super.isValidLetter(super.seqarr[i])) {
    			return false;
    		}
    }
    return true;
  ...
  */
  public char[] translate()
  {
    ...
    int i=0;
    int j=0;
    char[] protiens=new char[super.seqLength()];
    while(getAminoAcid(super.seqarr[i]+ super.seqarr[i+1]+ super.seqarr[i+2]+"")!='$' && i<super.seqLength()-3) {
    		protiens[j]=getAminoAcid(super.seqarr[i]+ super.seqarr[i+1]+ super.seqarr[i+2]+"");
    		j++;
    		i+=3;
    }
    if(super.seqLength()%3!=0) {
    		String temp="";
    		for(int k=super.seqLength()-super.seqLength()%3;k<super.seqLength();k++) {
    			temp+=super.seqarr[k];
    		}
    		protiens[j]=getAminoAcid(temp);
    }
    return protiens;
  }
